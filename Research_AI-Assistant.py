import re
import urllib.parse
# package name is 'ddgs' (duckduckgo_search renamed)
...
class ShortResearchAgent:
    ...
    def run(self, query):
        ...
        top_passages = [{"url": docs[i]["url"], "passage": docs[i]
                         ["passage"], "score": float(sims[i])} for i in top_idx]
        ...
        if not sentences:
            ...
            lines = []
            for s in chosen:
                # Check first 80 chars for duplication
                key = s["sent"].lower()[:80]
                if key in seen:
                    continue
                seen.add(key)
...
if __name__ == "__main__":
    ...
    q = "is today an easter in finlan?"
